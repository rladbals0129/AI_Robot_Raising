...
namespace SahurRaising.EditorTools
{
    ...
    public static class AutoAddLocalizedFontHandler
    {
        ...
        private static void TryAddLocalizedFont(GameObject go)
        {
            ...
            // 폰트 에셋 로드 (EditorPrefs에 저장된 경로 우선 사용, 없으면 기본값)
            string targetPath = EditorPrefs.GetString(ReplaceTMPFontsEditor.PrefKeyTargetFont, ReplaceTMPFontsEditor.DefaultTargetFontPath);
            string cjkPath = EditorPrefs.GetString(ReplaceTMPFontsEditor.PrefKeyCjkFont, ReplaceTMPFontsEditor.DefaultCjkFontPath);

            var koreanFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>(targetPath);
            var cjkFont = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>(cjkPath);
